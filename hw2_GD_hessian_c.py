...
X=torch.randn(100,1, out=None)
X1=torch.randn(100,1, out=None)
...
E,V=torch.eig(torch.matmul(A,torch.transpose(B,0,1))*2)
E1,V1=torch.eig(torch.matmul(A_tuta,torch.transpose(A_tuta,0,1))*2)
E2,V2=torch.eig(torch.matmul(A_tuta,torch.transpose(A,0,1))*2)
alpha=1/torch.max(E)
alpha1=1/torch.max(E1)
for iter in range(iteration_time):

    f = torch.pow(torch.norm(torch.matmul(torch.transpose(B,0,1),X)),2)
    g = torch.matmul(torch.matmul(B,torch.transpose(B,0,1)),X)*2
...
plt.subplot(1, 2, 1)
plt.plot(itr_time, Loss_list,label='GD with B')
...
plt.legend()
plt.subplot(1, 2, 2)
plt.plot(itr_time, Grad_norm,label='GD with B')
